...
namespace WoohooDX12
{
  ...
  {
    projMat = XMMatrixPerspectiveFovLH(XMConvertToRadians(float(fov)), float(viewWidth / viewHeight), nearPlane, farPlane);
    return projMat;
  }
  ...
  Mat Camera::CalculateViewMat()
  {
    ...
    viewMat           = XMMatrixLookAtLH(pos, XMVectorAdd(frontVec, pos), XMLoadFloat3(&worldUp));
    return viewMat;
  }

  // Setters
  void Camera::SetProperties(unsigned short fov_in_Angle, float aspect_Width, float aspect_Height, float sensitivity)
  {
    fov              = fov_in_Angle;
  ...
  void Camera::TakeInputs(float mouseOffsetX, float mouseOffsetY)
  {
    ...
    target          = XMVectorSet(cos(radPitch) * cos(radYaw), sin(radPitch), cos(radPitch) * sin(radYaw), 0.0f);

...
namespace WoohooDX12
{
  ...
  void Renderer::InitResources()
  {
    ...
    // Create the pipeline state
    {
      ...
      // Create the UBO
      {
        ...
        m_uniformBuffer->Create(m_device,
                                false,
                                BufferType::BT_ConstantBuffer,
                                (sizeof(uboVS) + 255) & ~255,
                                "Constant Buffer Default Heap");
    ...
    // Command lists are created in the recording state, but there is nothing to record yet. The main loop expects it to be
    // closed, so close it now.
    ...
    // Create synchronization objects and wait until assets have been uploaded to the GPU.
    {
      ...
      // Wait for the command list to execute; we are reusing the same command list in our main loop but for now, we just want to
  ...
  void Renderer::CompileShaders(ID3DBlob** vertexShader, ID3DBlob** pixelShader)
  {
    ...
    {
      ThrowIfFailed(
          D3DCompileFromFile(vertPath.c_str(), nullptr, nullptr, "main", "vs_5_0", compileFlags, 0, vertexShader, &errors));
      ThrowIfFailed(
          D3DCompileFromFile(fragPath.c_str(), nullptr, nullptr, "main", "ps_5_0", compileFlags, 0, pixelShader, &errors));
    }
    catch (std::exception e)
    {
